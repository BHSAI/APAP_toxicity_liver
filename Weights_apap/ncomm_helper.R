...
ef_df_cln = function(x) {
  ...
  x = ifelse(!is.na(x),as.character(x),"")                            # if there is no NA in x (if true) make them as characters else put an empty string in the place of NA
  if (!any(grepl("\\s",x))) return(x)                                 # if there is no space (\\s) in x if (true) return the x
  gsub("^\\s+|\\s+$","",x)                                            # if there is one or multiple spaces either at the begenning or end of string substite it with "" remove the space
}
